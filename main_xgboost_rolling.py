...
params = {"n_estimators": 600, "learning_rate": 0.03, "max_depth": 6, "subsample": 0.8, "colsample_bytree": 0.8, "reg_lambda": 1.0, "random_state": seed, "n_jobs": 1, "objective": "reg:squarederror", "eval_metric": "rmse", "tree_method": "hist", "verbosity": 0}
run_dir = os.path.join("Result", f"xgboost_rolling_{seed}")
...
for fold_id, test_year in enumerate(years[1:], start=1):
    ...
    fold_row = {"test_year": test_year, "n_train": int(len(tra_idx)), "n_val": int(len(val_idx)), "n_test": int(len(test_idx)), "best_val_hir_rrmse": float(min(model.evals_result()['validation_1']['rmse']) / max(float(np.mean(np.sort(y_val)[-max(1, int(len(y_val) * 0.2)):])), 1e-6)), "final_test_hir_rrmse": rrmse_metric_np(y_test, y_pred), "final_test_hir_rmse": rmse_metric_np(y_test, y_pred), "test_success_rate_0.15": sr015, "test_success_rate_0.15_n": n015, "test_success_rate_0.20": sr020, "test_success_rate_0.20_n": n020}
...
summary = {"run_dir": run_dir, "seed": seed, "model": "XGBoost_rolling", "years": fold_df["test_year"].tolist(), "precision_recall_thresholds": precision_recall_thresholds, "mean_test_hir_rrmse": float(fold_df["final_test_hir_rrmse"].mean()), "std_test_hir_rrmse": float(fold_df["final_test_hir_rrmse"].std(ddof=0)), "mean_test_hir_rmse": float(fold_df["final_test_hir_rmse"].mean()), "std_test_hir_rmse": float(fold_df["final_test_hir_rmse"].std(ddof=0)), "mean_test_precision_0.10": float(fold_df["test_precision_0.10"].mean()), "mean_test_recall_0.10": float(fold_df["test_recall_0.10"].mean()), "mean_test_precision_0.15": float(fold_df["test_precision_0.15"].mean()), "mean_test_recall_0.15": float(fold_df["test_recall_0.15"].mean()), "mean_test_precision_0.20": float(fold_df["test_precision_0.20"].mean()), "mean_test_recall_0.20": float(fold_df["test_recall_0.20"].mean())}
