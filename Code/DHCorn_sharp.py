...
def dhcorn(envi, mg, geno, pheno=0, device='cpu', stage_sharpness=0.12, start0_weight=1.0, repair_weight=0.5):
    ...
    v_AirStressCurve = torch.full((nsamples, 130), torch.nan, device=device)
    v_RootStressCurve = torch.full((nsamples, 130), torch.nan, device=device)
    v_HumidityStressCurve = torch.full((nsamples, 130), torch.nan, device=device)
    v_LightStressCurve = torch.full((nsamples, 130), torch.nan, device=device)
    v_HIStressLeafCurve = torch.full((nsamples, 130), torch.nan, device=device)
    v_HIStressStemCurve = torch.full((nsamples, 130), torch.nan, device=device)
    v_HIStressTasselCurve = torch.full((nsamples, 130), torch.nan, device=device)
    v_HIStressRootCurve = torch.full((nsamples, 130), torch.nan, device=device)
    ...
    for d in range(1, int(max(M_Harvest)) + 1):
        ...
        v_AirStressCurve[:, d] = v_AirStress
        v_RootStressCurve[:, d] = v_RootStress
        v_HumidityStressCurve[:, d] = v_HumidityStress
        v_LightStressCurve[:, d] = v_LightStress
        ...
        v_HIStressLeafCurve[:, d] = v_HIStressLeaf
        v_HIStressStemCurve[:, d] = v_HIStressStem
        v_HIStressTasselCurve[:, d] = v_HIStressTassel
        v_HIStressRootCurve[:, d] = v_HIStressRoot
        ...
        v_HIRate[:, d] = torch.tanh(start0_weight * g_start0 + g_start1 * v_HIPotential[:, d].clone()) * (1 - repair_weight * v_HIRepairRate)
    ...
    ph['stage'] = ts_all
    ph["AirStress"] = v_AirStressCurve
    ph["RootStress"] = v_RootStressCurve
    ph["HumidityStress"] = v_HumidityStressCurve
    ph["LightStress"] = v_LightStressCurve
    ph["HIStressLeaf"] = v_HIStressLeafCurve
    ph["HIStressStem"] = v_HIStressStemCurve
    ph["HIStressTassel"] = v_HIStressTasselCurve
    ph["HIStressRoot"] = v_HIStressRootCurve
