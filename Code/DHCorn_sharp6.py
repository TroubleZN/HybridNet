...
def dhcorn(envi, mg, geno, pheno=0, device='cpu', stage_sharpness=0.12):
    ...
    ts_all = torch.zeros(nsamples, n_stages, 130).to(device)  # (6)
    ...
    for d in range(1, int(max(M_Harvest)) + 1):
        ...
        stage_gates = [smooth_step(v_gpu_d, g_Stage[:, s], stage_sharpness) for s in range(1, n_stages)]

        ts_all[:, 0, d] = 1 - stage_gates[0]
        for s in range(1, n_stages - 1):
            ts_all[:, s, d] = stage_gates[s - 1] * (1 - stage_gates[s])
        ts_all[:, n_stages - 1, d] = stage_gates[-1]
        ts = ts_all[:, :, d].clone()
        ...
        v_HIRate[:, d] = 0*g_start0 + g_start1 * (1 / (1 + exp(- v_HIPotential[:, d].clone()))) * (1 - v_HIRepairRate)
