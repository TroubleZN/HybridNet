...
def dhcorn(envi, mg, geno, pheno=0, device='cpu'):
    ...
    # ph.LeafDry, nsamples by 180
    # ph.LeafWet, nsamples by 180
    # ph.StemDry, nsamples by 180
    # ph.StemWet, nsamples by 180
    # ph.Height, nsamples by 180
    # ph.Tiller, nsamples by 180
    ...
    ts = torch.zeros(nsamples, 3).to(device)  # (6)
    ...
    v_AccuAirStress = torch.zeros(nsamples, 180).to(device)
    v_AccuRootStress = torch.zeros(nsamples, 180).to(device)
    ...
    v_MWater = torch.zeros(nsamples, 180).to(device)  # (21)
    ...
    v_MBiomass = torch.zeros(nsamples, 180).to(device)  # (50)
    ...
    v_MLeafWeight = 0.1 * torch.ones(nsamples, 180).to(device)  # initial 1 gram (62)
    v_MRootWeight = 0.1 * torch.ones(nsamples, 180).to(device)  # initial 1 gram (63)
    v_MRootLength = 0.1 * torch.ones(nsamples, 180).to(device) / mean(g_RootDensity, 1).view(nsamples, 1).to(
        device)  # (64)
    v_MRootLayerLength = torch.zeros(nsamples, 180, 4).to(device)
    v_MRootLayerLength[:, 0, 0] = 0.1 / mean(g_RootDensity, 1).to(device)  # (65)
    v_MStemWeight = 0.1 * torch.ones(nsamples, 180).to(device)  # (66)
    v_MStemHeight = 0.1 * torch.ones(nsamples, 180).to(device) / mean(g_StemDensity, 1).view(nsamples, 1).to(
        device)  # (67)
    v_MGrainWeight = torch.zeros(nsamples, 180).to(device)  # (68)
    v_TasselHeight = torch.zeros(nsamples, 180).to(device)  # (68)
    v_TasselWeight = torch.zeros(nsamples, 180).to(device)  # (68)
    v_MShootWeight = v_MLeafWeight + v_MStemWeight + v_MGrainWeight + v_TasselWeight  # (69)

    v_HIRate = torch.zeros(nsamples, 180).to(device)
    v_HIPotential = torch.zeros(nsamples, 180).to(device)
    ...
    for d in range(1, int(max(M_Harvest)) + 1):
        ...
        v_GPU = v_GPU + sum(pow(v_GPUTemp, g_GPUTemp.view(nsamples, 1)) * pow(v_GPULight, g_GPULight.view(nsamples, 1)), 1)  # nsamples by 1. (5). feedback on g_Stage

        ts = ts.clone()
        ts[:, 0] = (v_GPU <= g_Stage[:, 1]) * (g_Stage[:, 1] - v_GPU) / g_Stage[:, 1]
        ts[:, 1] = (v_GPU <= g_Stage[:, 1]) * v_GPU / g_Stage[:, 1] \
                   + (v_GPU > g_Stage[:, 1]) * (v_GPU < g_Stage[:, 2]) * (g_Stage[:, 2] - v_GPU) / (
                               g_Stage[:, 2] - g_Stage[:, 1])
        ts[:, 2] = (v_GPU > g_Stage[:, 1]) * (v_GPU <= g_Stage[:, 2]) * (v_GPU - g_Stage[:, 1]) / (
                    g_Stage[:, 2] - g_Stage[:, 1]) \
                   + (v_GPU >= g_Stage[:, 2])  # nsamples by 3, (6)
        ...
        # v_AccuAirStress = v_AccuAirStress.clone()
        ...
        # new_val = torch.max(v_AccuAirStress[:, d - 1] - v_AccuAirStress[:, d - 1] * v_MShootWeight[:, d - 1] / sum(
        #     g_AccuAirStressRecovery * ts, 1) + v_AirStress, tensor(0))
        # v_AccuAirStress = v_AccuAirStress.clone()
        # v_AccuAirStress[:, d:d + 1] = new_val.unsqueeze(1)

        v_AccuRootStress = v_AccuRootStress.clone()
        ...
        temp6_3 = v_MRootLength[:, d - 1]
        temp6_4 = sum(E_SoilMoist[:, 24 * d:24 * d + 24, :], 1)
        v_RootWaterCap = temp6_1 * temp6_2 / M_Density * sum(temp6_3.view(nsamples, 1) * temp6_4,
                                                             1)  # nsamples by 1, (15)
        ...
        # Biomass
        v_PhotoLightCap = sum(g_Photo_Light * ts, 1) * sum(E_Light[:, 24 * d:24 * d + 24],
                                                           1) / M_Density * v_AccuAirStressIndex * v_LightDensityIndex  # nsamples by 1, (29)
        ...
        v_MPhotoLeafCap = sum(g_Photo_Leaf * ts, 1) * (1 - v_AirStress) * v_MLeafWeight[:, d - 1].clone() * v_AccuAirStressIndex  # nsamples by 1, (32)
        ...
        # v_MStemWeight[:, d] = v_MStemWeight[:, d - 1] + max(- v_MStemMS + v_MGrowth * fmax(v_MStemGrowthCap / v_MGrowthCap, tensor(0)), tensor(0))  # nsamples by 180, (66)
        # v_MStemWeight = v_MStemWeight.clone()
        v_MStemWeight[:, d] = v_MStemWeight[:, d - 1].clone() + max(- v_MStemMS + v_MGrowth * v_MStemGrowthCap / max(v_MGrowthCap, tensor(1)), tensor(0))  # nsamples by 180, (66)

        v_MStemHeight_t = v_MStemHeight[:, d - 1].clone()
        v_MStemHeight = v_MStemHeight.clone()
        v_MStemHeight[:, d] = max(v_MStemHeight_t, v_MStemWeight[:, d].clone() / sum(g_StemDensity * ts, 1))  # nsamples by 180, (67)

        # v_MGrainWeight = v_MGrainWeight.clone()
        v_MGrainWeight[:, d] = v_MGrainWeight[:, d - 1].clone() + max(- v_MGrainMS + v_MGrowth * v_MGrainGrowthCap / max(v_MGrowthCap, tensor(1)), tensor(0))  # nsamples by 180, (68)

        # v_TasselWeight = v_TasselWeight.clone()
        v_TasselWeight[:, d] = v_TasselWeight[:, d - 1].clone() + max(- v_TasselMS + v_MGrowth * v_TasselGrowthCap / max(v_MGrowthCap, tensor(1)), tensor(0))  # nsamples by 180, (66)

        v_TasselHeight_t = v_TasselHeight[:, d - 1].clone()
        v_TasselHeight = v_TasselHeight.clone()
        v_TasselHeight[:, d] = max(v_TasselHeight_t, v_TasselWeight[:, d].clone() / sum(g_TasselDensity * ts, 1))  # nsamples by 180, (67)

        # v_MShootWeight = v_MShootWeight.clone()
        v_MShootWeight[:, d] = v_MLeafWeight[:, d].clone() + v_MStemWeight[:, d].clone() + v_MGrainWeight[:, d].clone() + v_TasselWeight[:, d].clone()  # nsamples by 180, (69)

        # HI
        v_HIStressLeaf = max(tensor(0), 1 - sum(g_AirStressIndexLeaf * ts, 1) * v_AccuAirStress[:, d])
        v_HIStressStem = max(tensor(0), 1 - sum(g_AirStressIndexStem * ts, 1) * v_AccuAirStress[:, d])
        v_HIStressTassel = max(tensor(0), 1 - sum(g_AirStressIndexTassel * ts, 1) * v_AccuAirStress[:, d])
        v_HIStressRoot = max(tensor(0), 1 - sum(g_AirStressIndexRoot * ts, 1) * v_AccuRootStress[:, d])

        v_HIMatterCapLeaf = sum(g_HI_Leaf * ts, 1) * v_MLeafWeight[:, d] * v_HIStressLeaf
        v_HIMatterCapStem = sum(g_HI_Stem * ts, 1) * v_MStemWeight[:, d] * v_HIStressLeaf
        v_HIMatterCapTassel = sum(g_HI_Tassel * ts, 1) * v_TasselWeight[:, d] * v_HIStressTassel
        v_HIMatterCapRoot = sum(g_HI_Root * ts, 1) * v_MRootWeight[:, d] * v_HIStressRoot

        v_HIPotentialLeaf = max(tensor(0), (v_HIMatterCapLeaf - sum(g_HIReferenceThresholdLeaf * ts, 1)) / sum(g_HIReferenceThresholdLeaf * ts, 1))
        v_HIPotentialStem = max(tensor(0), (v_HIMatterCapStem - sum(g_HIReferenceThresholdStem * ts, 1)) / sum(g_HIReferenceThresholdStem * ts, 1))
        v_HIPotentialTassel = max(tensor(0), (v_HIMatterCapTassel - sum(g_HIReferenceThresholdTassel * ts, 1)) / sum(g_HIReferenceThresholdTassel * ts, 1))
        v_HIPotentialRoot = max(tensor(0), (v_HIMatterCapRoot - sum(g_HIReferenceThresholdRoot * ts, 1)) / sum(g_HIReferenceThresholdRoot * ts, 1))
        ...
        v_HIRate[:, d] = v_HIPotential[:, d] * (1 - v_HIRepairRate)


    index = tensor([i * 180 + M_Harvest.long()[i] - 1 for i in range(nsamples)]).to(device)
...
if __name__ == "__main__":
    ...
    for epoch in range(1000):
        ...
        ph = dhcorn(envi, mg, geno, device=device)
        loss = torch.nn.functional.mse_loss(ph["Height"], target_height) / ph["Height"].mean()
        loss.backward()
        optimizer.step()
        print(f"Epoch {epoch + 1:02d}, Height RRMSE Loss = {loss.item():.4f}")
