...
def make_treno():
    ...
    treno[:, :, id + 23] = 0.2  # TasselMS0
    treno[:, :, id + 24] = 0.5  # TasselMS1
    id = id + 3  # id = 60
    ...
    Ltreno[:, :, 1] = [0, 0, 0, 32]  # firt two are useless, min temp GDU
    Ltreno[:, :, 2] = [0, 0, 0, 70.01]  # firt two are useless, max temp GDU
    Ltreno[:, :, 3] = [0, 0, 0, 2e6]  # firt two are useless, max light GPU, max data = 3.7e7
    Ltreno[:, :, 4] = [0, 0, 0, 0]  # firt two are useless, power temp GPU
    Ltreno[:, :, 5] = [0, 0, 0, 0]  # firt two are useless, power light GPU
    ...
    Ltreno[:, :, id + 15] = 1e-6  # dGrain0
    Ltreno[:, :, id + 16] = 1e-6  # dGrain1
    id = id + 16  # id = 47
    ...
    Ltreno[:, :, id + 13] = 1e-6  # Maternal Health
    Ltreno[:, :, id + 14] = 1e-6  # Maternal Repair
    ...
    Ltreno[:, :, id + 23] = 1e-6  # HITasselMS0
    Ltreno[:, :, id + 24] = 1e-6  # HITasselMS1
    id = id + 14  # id = 74
    ...
    Utreno[:, :, 1] = [0, 0, 0, 40]  # firt two are useless, min temp GDU
    Utreno[:, :, 2] = [0, 0, 0, 120]  # firt two are useless, max temp GDU
    ...
    Utreno[:, :, 6] = [0, 700, 150, 150]  # additional GPU beyond previous stage
    id = 6
    ...
    Utreno[:, :, id + 23] = 0.4  # HITasselMS0
    Utreno[:, :, id + 24] = 2  # HITasselMS1
    id = id + 14  # id = 74
