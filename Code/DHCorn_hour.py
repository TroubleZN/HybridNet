...
def dhcorn(envi, mg, geno, pheno=0, device='cpu'):
    ...
    v_GPU = torch.zeros(nsamples).to(device)  # (5)
    ts_all = torch.zeros(nsamples, 4, 130*24).to(device)  # (6)
    ...
    v_MWater = torch.zeros(nsamples, 130*24).to(device)  # (21)
    ...
    v_MBiomass = torch.zeros(nsamples, 130*24).to(device)  # (50)
    ...
    v_MLeafWeight = 0.1 * torch.ones(nsamples, 130*24).to(device)  # initial 1 gram (62)
    v_MRootWeight = 0.1 * torch.ones(nsamples, 130*24).to(device)  # initial 1 gram (63)
    v_MRootLength = 0.1 * torch.ones(nsamples, 130*24).to(device) / mean(g_RootDensity, 1).view(nsamples, 1).to(
        device)  # (64)
    v_MRootLayerLength = torch.zeros(nsamples, 130*24, 4).to(device)
    v_MRootLayerLength[:, 0, 0] = 0.1 / mean(g_RootDensity, 1).to(device)  # (65)
    v_MStemWeight = 0.1 * torch.ones(nsamples, 130*24).to(device)  # (66)
    v_MStemHeight = 0.1 * torch.ones(nsamples, 130*24).to(device) / mean(g_StemDensity, 1).view(nsamples, 1).to(
        device)  # (67)
    v_MStemHeight[:, 1:] = torch.nan
    v_MGrainWeight = torch.zeros(nsamples, 130*24).to(device)  # (68)
    v_TasselHeight = torch.zeros(nsamples, 130*24).to(device)  # (68)
    v_TasselHeight[:, 1:] = torch.nan
    v_TasselWeight = torch.zeros(nsamples, 130*24).to(device)  # (68)
    v_MShootWeight = v_MLeafWeight + v_MStemWeight + v_MGrainWeight + v_TasselWeight  # (69)

    v_HIMatterCapLeaf = torch.zeros(nsamples, 130*24).to(device)
    v_HIMatterCapStem = torch.zeros(nsamples, 130*24).to(device)
    v_HIMatterCapRoot = torch.zeros(nsamples, 130*24).to(device)
    v_HIMatterCapTassel = torch.zeros(nsamples, 130*24).to(device)

    v_HIRate = torch.zeros(nsamples, 130*24).to(device)
    v_HIRate[:, 1:] = torch.nan
    v_HIPotential = torch.zeros(nsamples, 130*24).to(device)
    ...
    start = time()
    for d in range(1, int(max(M_Harvest)) + 24):
        # Phenology
        v_ARootTemp = sum(v_MRootLayerLength[:, d - 1, :] * E_SoilTemp[:, d, :], 1) \
                      / sum(v_MRootLayerLength[:, d - 1, :], 1)

        v_GDUTemp = (min(max((1 - g_GDURootTemp) * E_AirTemp[:, d] + g_GDURootTemp * v_ARootTemp,
                            g_MinTemp),
                        g_MaxTemp)
                     - g_MinTemp)  # nsamples by 24. (2)

        v_GPUTemp = max(v_GDUTemp / (g_MaxTemp - g_MinTemp), tensor(1e-16))  # nsamples by 24. (3)

        v_GPULight = max(min(E_Light[:, d] / g_MaxLight, tensor(1)), tensor(1e-16))  # nsamples by 24. (4)

        v_GPU = v_GPU + pow(v_GPUTemp, g_GPUTemp) * pow(v_GPULight, g_GPULight)  # nsamples by 1. (5). feedback on g_Stage

        ts_all[:, 0, d] = (v_GPU <= g_Stage[:, 1]) * (g_Stage[:, 1] - v_GPU) / g_Stage[:, 1]
        ts_all[:, 1, d] = (v_GPU <= g_Stage[:, 1]) * v_GPU / g_Stage[:, 1] \
                        + (v_GPU > g_Stage[:, 1]) * (v_GPU <= g_Stage[:, 2]) * (g_Stage[:, 2] - v_GPU) / (g_Stage[:, 2] - g_Stage[:, 1])
        ts_all[:, 2, d] = (v_GPU > g_Stage[:, 1]) * (v_GPU <= g_Stage[:, 2]) * (v_GPU - g_Stage[:, 1]) / (g_Stage[:, 2] - g_Stage[:, 1]) \
                        + (v_GPU > g_Stage[:, 2]) * (v_GPU <= g_Stage[:, 3]) * (g_Stage[:, 3] - v_GPU) / (g_Stage[:, 3] - g_Stage[:, 2])
        ts_all[:, 3, d] = (v_GPU > g_Stage[:, 2]) * (v_GPU <= g_Stage[:, 3]) * (v_GPU - g_Stage[:, 2]) / (g_Stage[:, 3] - g_Stage[:, 2]) \
                        + (v_GPU >= g_Stage[:, 3])  # nsamples by 3, (6)
        ts = ts_all[:, :, d].clone()

        # Stress
        temp1 = E_AirTemp[:, d] - sum(g_BestAirTemp * ts, 1)

        v_AirHeat = safe_pow(min(max(temp1, tensor(0)) / sum((g_MaxAirTemp - g_BestAirTemp) * ts, 1), tensor(1)),
                             sum(g_AirHeatPower * ts, 1))

        v_AirCold = safe_pow(min(min(temp1, tensor(0)) / sum((g_MinAirTemp - g_BestAirTemp) * ts, 1), tensor(1)),
                             sum(g_AirColdPower * ts, 1))

        temp2 = v_ARootTemp - sum(g_BestRootTemp * ts, 1)
        temp3 = sum(g_RootHeatPower * ts, 1)
        temp3_2 = sum((g_MaxRootTemp - g_BestRootTemp) * ts, 1)
        temp4 = sum(g_RootColdPower * ts, 1)
        temp4_2 = sum((g_MinRootTemp - g_BestRootTemp) * ts, 1)

        v_RootHeat = safe_pow(min(max(temp2, tensor(0)) / temp3_2, tensor(1)), temp3)
        v_RootCold = safe_pow(min(min(temp2, tensor(0)) / temp4_2, tensor(1)), temp4)
        ...
        humid = E_Humidity[:, d] - sum(g_BestHumidity * ts, 1)

        v_AirWet = safe_pow(min(max(humid, tensor(0)) / sum((g_MaxHumidity - g_BestHumidity) * ts, 1), tensor(1)),
                            sum(g_AirWetPower * ts, 1))

        v_AirDry = safe_pow(min(min(humid, tensor(0)) / sum((g_MinHumidity - g_BestHumidity) * ts, 1), tensor(1)),
                            sum(g_AirDryPower * ts, 1))

        v_HumidityStress = 1 - (1 - v_AirDry) * (1 - v_AirWet)

        light = E_Light[:, d] - sum(g_BestLight * ts, 1)

        v_RadiationStress = safe_pow(min(max(light, tensor(0)) / sum((g_MaxLightS - g_BestLight) * ts, 1), tensor(1)),
                                     sum(g_RadiationPower * ts, 1))

        v_ShadingStress = safe_pow(min(min(light, tensor(0)) / sum((g_MinLightS- g_BestLight) * ts, 1), tensor(1)),
                                   sum(g_ShadingPower * ts, 1))
        ...
        v_RootWaterCap = (1 - v_RootStress) * sum(g_RootWaterCap * ts, 1) / M_Density * sum(v_MRootLayerLength[:, d - 1, :] * E_SoilMoist[:, d, :], 1)


        v_MWaterUptakeCap = max(v_MXylemCap - (v_MWater[:, d] - v_MTranspiration - v_MWaterPhoto), tensor(0))  # nsamples by 1, (16)
        ...
        v_MWater[:, d] = min(v_MXylemCap,
                             v_MWater[:, d - 1] - v_MTranspiration - v_MWaterPhoto + v_MWaterUptake)  # nsamples by 1, (21)

        # Transpiration
        v_TempTranspiration = 1 - exp(min(32 - E_AirTemp[:, d], tensor(0)) * sum(g_TempTranspiration * ts, 1))

        v_HumidityTranspiration = exp(-E_Humidity[:, d] * sum(g_HumidityTranspiration * ts, 1))  # nsamples by 1, (24)

        v_EvapTranspiration = 1 - sum(g_EvapTranspiration0 * ts, 1) * exp(-(E_Evap[:, d] + 2.52e-3) * sum(g_EvapTranspiration1 * ts, 1))

        v_WindTranspiration = 1 - sum(g_WindTranspiration0 * ts, 1) * exp(-(E_Wind[:, d]) * sum(g_WindTranspiration1 * ts, 1))
        ...
        # Biomass
        v_PhotoLightCap = sum(g_Photo_Light * ts, 1) * E_Light[:, d] / M_Density * (1 - v_LightStress)

        v_MPhotoWaterCap = (sum(g_Photo_Water * ts, 1) *
                            min(
                                v_MWater[:, d] - v_MTranspiration,
                                sum(g_PhotoWater_Transpiration * ts, 1) * v_MTranspiration
                            ))
        ...
        v_MPhoto = min(torch.stack([v_PhotoLightCap, v_MPhotoWaterCap, v_MPhotoLeafCap, v_MPhloemCap - v_MBiomass[:, d - 1].clone()], -1), 1)[0]  # nsamples by 1, (36)

        v_MLeafGrowthCap = ((1 - v_AirStress) *
                            min(
                                sum(g_dLeaf0 * ts, 1) + sum(g_dLeaf1 * ts, 1) * v_MLeafWeight[:, d - 1].clone() + v_MLeafMS,
                                sum(g_Phloem_Leaf * ts, 1) * v_MLeafWeight[:, d - 1].clone()
                                )
                            )

        v_MRootGrowthCap = (1 - v_RootStress) * (sum(g_dRoot0 * ts, 1) + sum(g_dRoot1 * ts, 1) * v_MRootWeight[:, d - 1].clone() + v_MRootMS)  # nsamples by 1, (39)

        v_MStemGrowthCap = ((1 - v_AirStress) *
                            min(
                                sum(g_dStem0 * ts, 1) + sum(g_dStem1 * ts, 1) * v_MStemWeight[:, d - 1].clone() + v_MStemMS,
                                sum(g_Phloem_Stem * ts, 1) * v_MStemWeight[:, d - 1].clone()
                                )
                            )

        v_MGrainGrowthCap = (1 - v_AirStress) * (sum(g_dGrain0 * ts, 1) + sum(g_dGrain1 * ts, 1) * v_MGrainWeight[:, d - 1].clone() + v_MGrainMS)  # nsamples by 1, (41)

        v_TasselGrowthCap = (1 - v_AirStress) * (sum(g_dTassel0 * ts, 1) + sum(g_dTassel1 * ts, 1) * v_TasselWeight[:, d - 1].clone() + v_TasselMS)  # nsamples by 1, (41)
        ...
        v_MLeafWeight[:, d] = v_MLeafWeight[:, d - 1].clone() + max(- v_MLeafMS + v_MGrowth * v_MLeafGrowthCap / max(v_MGrowthCap, tensor(1)), tensor(0))

        v_MRootWeight[:, d] = v_MRootWeight[:, d - 1].clone() + max(- v_MRootMS + v_MGrowth * v_MRootGrowthCap / max(v_MGrowthCap, tensor(1)), tensor(0))

        v_MRootLength[:, d] = max(v_MRootLength[:, d - 1].clone(), v_MRootWeight[:, d].clone() / sum(g_RootDensity * ts, 1))  # nsamples by 1, (64)
        ...
        v_MStemHeight[:, d] = max(v_MStemHeight[:, d - 1].clone(), v_MStemWeight[:, d].clone() / sum(g_StemDensity * ts, 1))  # nsamples by 130, (67)
        ...
        v_TasselHeight[:, d] = max(v_TasselHeight[:, d - 1].clone(), v_TasselWeight[:, d].clone() / sum(g_TasselDensity * ts, 1))  # nsamples by 130, (67)
        ...
        v_HIRepairRate = exp(- sum(g_MaternalHealth * ts, 1) - sum(g_MaternalRepair * ts, 1))

        v_HIRate = v_HIRate.clone()
        v_HIRate[:, d] = g_start0 + g_start1 * (1 / (1 + exp(- v_HIPotential[:, d].clone()))) * (1 - v_HIRepairRate)

    index = tensor([i * 130 + M_Harvest.long()[i] - 1 for i in range(nsamples)]).to(device)

    HarvestHeight = v_MStemHeight.take(index)  # in meter

    ph["Height"] = v_MStemHeight
    ph['HIR'] = v_HIRate
