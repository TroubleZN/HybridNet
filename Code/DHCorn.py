...
def dhcorn(envi, mg, geno, pheno=0, device='cpu'):
    ...
    for d in range(1, int(max(M_Harvest)) + 1):
        ...
        ts_all[:, 0, d] = (v_GPU[:, d].clone() <= g_Stage[:, 1]) * (g_Stage[:, 1] - v_GPU[:, d].clone()) / g_Stage[:, 1]
        ts_all[:, 1, d] = (v_GPU[:, d].clone() <= g_Stage[:, 1]) * v_GPU[:, d].clone() / g_Stage[:, 1] \
                        + (v_GPU[:, d].clone() > g_Stage[:, 1]) * (v_GPU[:, d].clone() <= g_Stage[:, 2]) * (g_Stage[:, 2] - v_GPU[:, d].clone()) / (g_Stage[:, 2] - g_Stage[:, 1])
        ts_all[:, 2, d] = (v_GPU[:, d].clone() > g_Stage[:, 1]) * (v_GPU[:, d].clone() <= g_Stage[:, 2]) * (v_GPU[:, d].clone() - g_Stage[:, 1]) / (g_Stage[:, 2] - g_Stage[:, 1]) \
                        + (v_GPU[:, d].clone() > g_Stage[:, 2]) * (v_GPU[:, d].clone() <= g_Stage[:, 3]) * (g_Stage[:, 3] - v_GPU[:, d].clone()) / (g_Stage[:, 3] - g_Stage[:, 2])
        ts_all[:, 3, d] = (v_GPU[:, d].clone() > g_Stage[:, 2]) * (v_GPU[:, d].clone() <= g_Stage[:, 3]) * (v_GPU[:, d].clone() - g_Stage[:, 2]) / (g_Stage[:, 3] - g_Stage[:, 2]) \
                        + (v_GPU[:, d].clone() >= g_Stage[:, 3])  # nsamples by 3, (6)
        ts = ts_all[:, :, d].clone()
        ...
        v_HIRate[:, d] = torch.tanh(g_start0 + g_start1 * v_HIPotential[:, d].clone()) * (1 - v_HIRepairRate)
