...
def precision_recall_at_threshold(pred, true, threshold):
    ...
    if pred_n == 0:
        ...
        precision = float(np.mean(true[pred_pos] > threshold))
    if actual_n == 0:
        recall = float("nan")
    else:
        recall = float(np.mean(pred[actual_pos] > threshold))
...
env_features = torch.cat([env_w.mean(dim=2), env_w.max(dim=2)[0], env_w.min(dim=2)[0]], dim=2).reshape(env.shape[0], -1)
X_all = torch.cat([mg, G, env_features], dim=1).cpu().numpy()
y_all = HIR.cpu().numpy()

run_name = f"xgboost_loyo_{seed}"
...
for fold_id, test_year in enumerate(years, start=1):
    ...
    fold_row = {
        ...
        "best_val_hir_rrmse": float(min(model.evals_result()["validation_1"]["rmse"]) / max(float(np.mean(np.sort(y_val)[-max(1, int(len(y_val) * 0.2)):])), 1e-6)),
        "final_train_hir_rrmse": rrmse_metric_np(y_train, y_train_pred),
        "final_val_hir_rrmse": rrmse_metric_np(y_val, y_val_pred),
        "final_test_hir_rrmse": rrmse_metric_np(y_test, y_test_pred),
        "final_train_hir_rmse": rmse_metric_np(y_train, y_train_pred),
        "final_val_hir_rmse": rmse_metric_np(y_val, y_val_pred),
        "final_test_hir_rmse": rmse_metric_np(y_test, y_test_pred),
        "test_success_rate_0.15": success_rate_at_threshold(y_test_pred, y_test, 0.15)[0],
        "test_success_rate_0.15_n": success_rate_at_threshold(y_test_pred, y_test, 0.15)[1],
        "test_success_rate_0.20": success_rate_at_threshold(y_test_pred, y_test, 0.20)[0],
        "test_success_rate_0.20_n": success_rate_at_threshold(y_test_pred, y_test, 0.20)[1],
    }
    for threshold in precision_recall_thresholds:
        threshold_suffix = f"{threshold:.2f}"
        precision, recall, pred_n, actual_n = precision_recall_at_threshold(y_test_pred, y_test, threshold)
