...
def rrmse_metric_np(y_true, y_pred):
    ...
    top_pred_mean = max(float(np.mean(y_true[top_idx])), 1e-6)
    return rmse_metric_np(y_true, y_pred) / top_pred_mean
...
history_df["train_hir_rrmse"] = history_df["train_hir_rmse"] / max(np.mean(np.sort(y_train)[-max(1, int(len(y_train) * 0.2)):]), 1e-6)
history_df["val_hir_rrmse"] = history_df["val_hir_rmse"] / max(np.mean(np.sort(y_val)[-max(1, int(len(y_val) * 0.2)):]), 1e-6)
history_df["test_hir_rrmse"] = history_df["test_hir_rmse"] / max(np.mean(np.sort(y_test)[-max(1, int(len(y_test) * 0.2)):]), 1e-6)
