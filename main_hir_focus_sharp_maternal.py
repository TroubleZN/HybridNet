...
from Code.DHCorn_sharp_maternal import dhcorn
...
maternal_base = torch.zeros(2, device=device)
maternal_base.requires_grad_(True)
maternal_offset = torch.zeros(len(female_levels), 2, device=device)
maternal_offset.requires_grad_(True)

weight = torch.tensor([0.05, 0.90, 0.01, 0.05], device=device)
train_subset_ratio = 1.00
lr = 1e-4
beta_reg = 30
maternal_base_reg = 1.0
maternal_offset_reg = 1.0
grad_clip_norm = 1.0
max_epochs = 800
...
maternal_base_best = maternal_base.detach().clone()
maternal_offset_best = maternal_offset.detach().clone()
history = []

result_root = "Result"
os.makedirs(result_root, exist_ok=True)
run_name = f"hir_focus_sharp_maternal_{seed}"
...
optimizer = torch.optim.Adam([beta, maternal_base, maternal_offset], lr=lr)
...
for epoch in range(max_epochs):
    ...
    maternal_effect = maternal_base.unsqueeze(0) + torch.matmul(F, maternal_offset)
    ph = dhcorn(env, mg, geno, maternal_effect=maternal_effect, device=device, stage_sharpness=stage_sharpness)
    ...
    loss = (
        ...
        + maternal_base.square().mean() * maternal_base_reg
        + maternal_offset.square().mean() * maternal_offset_reg
    )
    if not torch.isfinite(loss):
        print(f"Epoch {epoch + 1:02d}: non-finite loss, stop training")
        break
    loss.backward()
    torch.nn.utils.clip_grad_norm_([beta, maternal_base, maternal_offset], grad_clip_norm)
    optimizer.step()
    ...
    with torch.no_grad():
        ...
        maternal_base_l2 = maternal_base.detach().pow(2).mean().sqrt().item()
        maternal_offset_l2 = maternal_offset.detach().pow(2).mean().sqrt().item()
        history_row = {
            "epoch": epoch + 1,
            "lr": lr,
            "fail": fail,
            ...
            "beta_l2": beta_l2,
            "maternal_base_l2": maternal_base_l2,
            "maternal_offset_l2": maternal_offset_l2,
        }
            ...
            torch.save(
                {
                    ...
                    "maternal_base": maternal_base_best.detach().cpu(),
                    "maternal_offset": maternal_offset_best.detach().cpu(),
...
with torch.no_grad():
    ...
    maternal_base.copy_(maternal_base_best)
    maternal_offset.copy_(maternal_offset_best)
    g = (
        torch.matmul(G, beta[:, 0, :]).reshape(N, n_stages, 89)
        + torch.matmul((G == 1) * 1.0, beta[:, 1, :]).reshape(N, n_stages, 89)
    )
    ...
    config = {
        ...
        "beta_reg": beta_reg,
        "maternal_base_reg": maternal_base_reg,
        "maternal_offset_reg": maternal_offset_reg,
    ...
    torch.save(
        {
            ...
            "maternal_base": maternal_base.detach().cpu(),
            "maternal_offset": maternal_offset.detach().cpu(),
