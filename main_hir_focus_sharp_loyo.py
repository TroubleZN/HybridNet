...
run_dir = os.path.join("Result", os.environ.get("DHCORN_RUN_NAME", f"hir_focus_sharp_loyo_{seed}"))
...
for fold_id, test_year in enumerate(years, start=1):
    ...
    optimizer = torch.optim.Adam([beta], lr=lr_init)
    lr = lr_init
    fail = 0
    best_val = 1e9
    history = []
    ...
    for epoch in range(max_epochs):
        ...
        if not torch.isfinite(loss):
            print(f"Fold {fold_id} year {test_year}: non-finite loss, stop fold")
            break
        loss.backward()
